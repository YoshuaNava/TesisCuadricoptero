...
class HiloJoystick:
    ...
    def enviarComandosVentana(self):
                ...
                for evento in eventos:                
                    if evento.type == pygame.JOYAXISMOTION:
                        ...
                        if (movimientoX_ruedaDerecha != 0) or (movimientoY_ruedaDerecha != 0):
                            ...
                            self.comandoPitch = int(movimientoY_ruedaDerecha*self.MAXIMO_ANGULO)
                            self.comandoRoll = int(movimientoX_ruedaDerecha*self.MAXIMO_ANGULO)
                            #print 'Comando de pitch= %d' %(self.comandoPitch)
                            #print 'Comando de roll= %d' %(self.comandoRoll)
                        
                        self.enviarComandosVentana()
